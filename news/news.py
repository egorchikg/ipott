...
def insert_into_news(valusa):
    ...
    cursor = connection.cursor()
    #
    sql = f"INSERT INTO news VALUES ({valusa});"
    cursor.execute(sql)
    #
    connection.commit()
    #
...
def delete_from_news(valusa):
    ...
    cursor = connection.cursor()
    #
    sql = f"DELETE FROM news WHERE {valusa};"
    cursor.execute(sql)
    #
    connection.commit()
    #
