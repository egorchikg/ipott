...
def get_news_list(da):
    ...
    if(da == None):
        ...
        dt = datetime.datetime.strptime(da,"%Y.%m.%d")
    #
    kura_day = dt.strftime("%Y.%m.%d")
    #
    hedelu = get_news_dict_list()
    #
    folusa += get_news_from_news_dict_list(
        hedelu,
        kura_day,
    )
    #
...
def get_info_from_info_dict_list(
  ...
  return(output)
#
def get_dates_between(daka,dade):
    #
    nedalu = []
    nedalu.append(daka)
    #
    dakadt = datetime.datetime.strptime(daka,"%Y.%m.%d")
    dadedt = datetime.datetime.strptime(dade,"%Y.%m.%d")
    #
    da = dakadt
    while(da < dadedt):
      da = da + datetime.timedelta(days=1)
      sa = da.strftime("%Y.%m.%d")
      nedalu.append(sa)
    #
    return(nedalu)
    #
#
def get_news_from_news_dict_list(
  news_dict_list,
  kura_day,
):
  #
  ...
  #
  while(n < len(ru)):
    image = ru[n].get("image")
    daka = ru[n].get("daka")
    dade = ru[n].get("dade")
    dakadese = get_dates_between(daka,dade)
    if(kura_day in dakadese):
      #
      #
      if(boroso):
        #
